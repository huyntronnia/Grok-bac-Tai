...
const { getConversationStateScript } = require("./chatgpt_dom");

function getChatGptSendState(sceneId) {
  const key = sceneId ? String(sceneId) : "unknown";
  return globalThis.chatGptSendStates.get(key) || "NOT_STARTED";
}
...
async function getConversationState(page) {
  ...
  return evaluateOnCdpPage(
    page,
    `(${getConversationStateScript.toString()})()`,
  ).then((state) => {
    if (state && state.composerState) {
      globalThis.__vidoraLastComposerState = state.composerState;
    }
    return state;
  }).catch((error) => ({
    ok: false,
...
async function evaluateOnCdpPage(client, expression) {
  async function runEvaluate(expr, returnByValue = true) {
    ...
    if (result.exceptionDetails) {
      ...
      throw new Error(
        `${details.text || "CDP evaluate lỗi"}: ${description}${location !== "::" ? ` @ ${location}` : ""}`,
      );
    }
  ...
  try {
    ...
    if (
      !/Object reference chain is too long|Object couldn't be returned by value|Converting circular structure/i.test(
        message,
      )
    ) {
      throw error;
    ...
    try {
      ...
      throw new Error(
        `CDP safe-eval parse failed after "${message}": ${parseError.message}`,
      );
    }
