...
function queueAutoContinue(delay = 350) {
  if (project?.workflowMode === 'manual_keyframe_motion') return;
...
async function recoverWorkflowRun() {
  if (project?.workflowMode === 'manual_keyframe_motion') return window.startManualGptWorkflow?.();
  if (!project) {
...
async function maybeRunVeoUpAutomationAfterPipeline(reason = 'pipeline-complete', runId = getActivePipelineRunId()) {
  if (runId) assertPipelineRunActive(runId);
  if (project?.workflowMode === 'manual_keyframe_motion' || (manualChatGptBtn && manualChatGptBtn.getAttribute('aria-pressed') === 'true')) return;
...
async function startPipelineFromClick(event) {
  event?.preventDefault?.();
  return window.startManualGptWorkflow?.();
}
...
async function autoRunRoute() {
  ...
  try {
    ...
    if (project?.workflowMode === 'manual_keyframe_motion') {
...
// The hidden router controls belong to the retired automatic pipeline.
// Manual workflow commands are registered by manual_workflow_ui.js.
...
// Manual production UI is a command/view-model client; the main process owns capture.
const manualWorkflowUI = window.createManualWorkflowUI({
  api: window.videoPlannerAPI,
  document,
  getProjectPath: () => outputFolder,
  beforeStart: async () => {
    if (!project?.scenes?.length) throw new Error('Tạo hoặc mở project trước.');
    if (!outputFolder) await chooseOutputFolder();
    if (!outputFolder) throw new Error('Chưa có thư mục project.');
    if (currentProjectFilePath) await flushProjectAutosave({ force: true, reason: "manual-start" });
    else await saveProjectSessionFlow({ silent: true });
  },
  onError: (error) => setStatus(error?.message || String(error), 'error'),
});
window.startManualGptWorkflow = () => manualWorkflowUI.start();
window.videoPlannerAPI?.setWorkflowMode('manual_keyframe_motion').catch((error) => setStatus(error.message, 'error'));
