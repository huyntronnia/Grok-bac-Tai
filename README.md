...
Phục hồi chỉ F5 đúng chat hiện tại và kiểm tra conversation ID; pipeline không chọn chat ở sidebar hay điều hướng sang URL chat khác. Navigation tạm thời làm CDP mất execution context sẽ được chờ ổn định tại chỗ, không bị nhận nhầm là tab crash. Với run dài, Vidora refresh đúng conversation hiện tại sau mỗi 5 scene ChatGPT hoàn tất để giải phóng tài nguyên trang, không tự tạo chat mới. Nếu ChatGPT virtualize DOM làm chỉ số image-turn cũ vượt quá số turn đang mount, Vidora chỉ rebase tới image card nằm sau đúng NV1 user turn và có ownership hash hợp lệ. Nếu ChatGPT trả lời text-only cho NV1, Vidora xác minh assistant thuộc đúng user turn và đã ổn định trước khi gửi `RETRY 1` đúng một lần trong cùng conversation. Ảnh keyframe được giữ bằng đường dẫn file trên đĩa thay vì nhúng base64 vào state/IPC. Sau hai chu kỳ phục hồi scene không thành công, app lưu checkpoint và dừng an toàn.
